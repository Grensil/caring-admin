...
class LoginViewModel(
    ...
    val savedPw: String = adminStorage.read("admin_pw") ?: ""
    val autoLoginEnabled: Boolean = adminStorage.read("admin_auto_login") == "true"

    init {
        if (autoLoginEnabled && savedId.isNotEmpty() && savedPw.isNotEmpty()) {
            login(savedId, savedPw, saveLogin = false)
        }
    }
                ...
                .onSuccess { dto ->
                    if (dto.result == "success" && dto.token != null) {
                        if (saveLogin) {
                            ...
                            adminStorage.save("admin_pw", pw)
                            adminStorage.save("admin_auto_login", "true")
                        } else if (!autoLoginEnabled) {
                            // 자동 로그인 해제 시 저장된 정보 삭제
                            adminStorage.delete("admin_id")
                            adminStorage.delete("admin_pw")
                            adminStorage.delete("admin_auto_login")
                        }
